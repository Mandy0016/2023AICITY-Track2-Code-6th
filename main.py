...
from transformers import AutoTokenizer, XLMRobertaXLModel, BertTokenizer, RobertaTokenizer, RobertaModel
...
def prepare_start():
    ...
    parser.add_argument('--logs-dir', type=str,
                        metavar='PATH', default='logs/')
    parser.add_argument('--eval_only', '-eval',
                        action='store_true', help='only eval')
    parser.add_argument(
        "opts",
...
def evaluate_by_test_all(model, valloader, epoch, cfg, index=-1, args=None, tokenizer=None, optimizer=None):
    ...
    with torch.no_grad():
        for q_id in tqdm(queries.keys()):
            ...
            if 'dual-text' in cfg.MODEL.NAME:
                car_tokens = tokenizer.batch_encode_plus(
                    car_text, padding='longest', return_tensors='pt')
                ...
                lang_embeds_list = model.module.encode_text(
    ...
    with torch.no_grad():
        ...
        for q_id in all_visual_embeds[-1].keys():
            for feat_idx in range(feat_num):
                visual_embeds[feat_idx].append(
                    all_visual_embeds[feat_idx][q_id])
        ...
        for q_id in tqdm(all_visual_embeds[-1].keys()):
            for feat_idx in range(feat_num):
                ...
                cur_sim = torch.mean(torch.matmul(
                    lang_embeds, visual_embeds[feat_idx].T), 0, keepdim=True)
    ...
    def compute_and_record(sim, name):
        ...
        loss_t_2_i = F.cross_entropy(
            sim_t_2_i, torch.arange(sim_t_2_i.size(0)).cuda())
        loss_i_2_t = F.cross_entropy(
            sim_i_2_t, torch.arange(sim_t_2_i.size(0)).cuda())
        loss = (loss_t_2_i + loss_i_2_t) / 2

        acc1, acc5 = accuracy(sim_t_2_i, torch.arange(
            sim_t_2_i.size(0)).cuda(), topk=(1, 5))
    ...
    sum_mrr = compute_and_record(
        sum_sim, args.logs_dir.split('/')[-1] + 'feat_all')

    if args.eval_only and cfg.TEST.RERANK:
        rerank_mrr, rerank_params, rerank_sim = rerank_params_grid_search(
            all_lang_embeds[-1], all_visual_embeds[-1])
        print(
            f"====> grid search rerank, best mrr = {rerank_mrr}, params: k1={rerank_params[0]}, k2={rerank_params[1]}, eps={rerank_params[2]}")

    evl_end_time = time.monotonic()
    print(f'Epoch {epoch} running time: ', timedelta(
        seconds=evl_end_time - evl_start_time))
...
def evaluate_by_test(model, valloader, epoch, cfg, index=-1, args=None, tokenizer=None, optimizer=None):
    ...
    with torch.no_grad():
        ...
        for q_id in tqdm(all_visual_embeds.keys()):
            ...
            cur_sim = torch.mean(torch.matmul(
                lang_embeds, visual_embeds.T), 0, keepdim=True)
            all_sim.append(cur_sim)

    if args.eval_only and cfg.TEST.RERANK:
        rerank_mrr, rerank_params, rerank_sim = rerank_params_grid_search(
            all_lang_embeds, all_visual_embeds)
        print(
            f"====> grid search rerank, best mrr = {rerank_mrr}, params: k1={rerank_params[0]}, k2={rerank_params[1]}, eps={rerank_params[2]}")
    ...
    acc1, acc5 = accuracy(all_sim, torch.arange(
        all_sim.size(0)).cuda(), topk=(1, 5))
    ...
    results_record(args.logs_dir.split(
        '/')[-1], valloader.dataset.name, epoch, loss.item(), all_mrr, acc1[0], acc5[0], is_test=True)
    if args.eval_only:
        return
...
def main():
    ...
    val_data_test = CityFlowNLInferenceDataset(
        cfg.DATA, transform=transform_test, val=True)
    ...
    if args.eval_only:
        evaluate_by_test_all(model, valloader, 0, cfg,
                             args.feat_idx, args, tokenizer, optimizer)
        return
    ...
    if metric_learning == 'CircleLoss':
        ...
        metric_loss_f = CircleLoss(
            m=cfg.MODEL.METRIC.LOSS_MARGIN, gamma=cfg.MODEL.METRIC.LOSS_SCALE)
        ...
        metric_loss_f = CirclePairLoss(
            s=cfg.MODEL.METRIC.LOSS_SCALE, m=cfg.MODEL.METRIC.LOSS_MARGIN)
    elif metric_learning == 'PairCosFace':
        metric_loss_f = CosFacePairLoss(
            s=cfg.MODEL.METRIC.LOSS_SCALE, m=cfg.MODEL.METRIC.LOSS_MARGIN)
    else:
        metric_loss_f = None
    ...
    for epoch in range(cfg.TRAIN.EPOCH):
        if cfg.EVAL.EVAL_BY_TEST and (epoch + 1) % (cfg.EVAL.EPOCH * cfg.EVAL.EVAL_BY_TEST_NUM) == 0:
            evaluate_by_test(model, valloader, epoch, cfg,
                             args.feat_idx, args, tokenizer, optimizer)
        ...
        nlp_view_losses = AverageMeter(
            f'Nlp_View_{cfg.MODEL.HEAD.NLP_VIEW_LOSS}_Loss', ':.4e')
        mrr = AverageMeter('MRR', ':6.4f')
        top1_acc = AverageMeter('Acc@1', ':6.4f')
        top5_acc = AverageMeter('Acc@5', ':6.4f')
        progress = ProgressMeter(
            len(trainloader)*cfg.TRAIN.ONE_EPOCH_REPEAT,
            [batch_time, data_time, losses, clip_losses, cls_losses,
                metric_losses, nlp_view_losses, mrr, top1_acc, top5_acc],
            prefix="Epoch: [{}]".format(epoch))
        end = time.time()
        epo_start_time = time.monotonic()
        for tmp in range(cfg.TRAIN.ONE_EPOCH_REPEAT):
            model_freeze.on_train_epoch_start(
                epoch=epoch * cfg.TRAIN.ONE_EPOCH_REPEAT + tmp)
            for batch_idx, batch in enumerate(trainloader):
                image, text, car_text, view_text = batch["crop_data"], batch[
                    "text"], batch["car_text"], batch["view_text"]
                ...
                if cfg.DATA.USE_MOTION:
                    ...
                    if 'dual-text' in cfg.MODEL.NAME:
                        ...
                        if 'view' in cfg.MODEL.NAME:
                            view_tokens = tokenizer.batch_encode_plus(
                                view_text, padding='longest', return_tensors='pt')
                            outputs = model(tokens['input_ids'].cuda(), tokens['attention_mask'].cuda(),
                                            car_tokens['input_ids'].cuda(
                            ), car_tokens['attention_mask'].cuda(),
                                view_tokens['input_ids'].cuda(
                            ), view_tokens['attention_mask'].cuda(),
                ...
                if cfg.MODEL.HEAD.CLIP_LOSS:
                    for visual_embeds, lang_embeds in loss_pairs:
                        sim_i_2_t = torch.matmul(
                            visual_embeds, torch.t(lang_embeds))
                        acc_sim = sim_i_2_t.clone().detach()
                        sim_i_2_t = sim_i_2_t - \
                            (torch.eye(image.size(0)).cuda()
                             * cfg.MODEL.HEAD.CLIP_LOSS_MARGIN)
                        sim_i_2_t = torch.mul(logit_scale, sim_i_2_t)
                        sim_t_2_i = sim_i_2_t.t()
                        loss_t_2_i = F.cross_entropy(
                            sim_t_2_i, torch.arange(image.size(0)).cuda())
                        loss_i_2_t = F.cross_entropy(
                            sim_i_2_t, torch.arange(image.size(0)).cuda())
                        clip_loss += (loss_t_2_i+loss_i_2_t)/2
                else:
                    acc_sim = torch.matmul(
                        pairs[-1][0], torch.t(pairs[-1][1])).detach()
                ...
                if 'view' in cfg.MODEL.NAME:
                    ...
                    # motion pair
                    motion_vis_embeds, motion_lang_embeds = pairs[1]
                    if cfg.MODEL.HEAD.NLP_VIEW_LOSS == 'Triplet':
                        pos_sim = torch.diag(torch.matmul(
                            motion_vis_embeds, motion_lang_embeds.T)).unsqueeze_(1)
                        neg_sim = torch.diag(torch.matmul(
                            motion_vis_embeds, view_lang_embeds.T)).unsqueeze_(1)
                        delta = neg_sim - pos_sim + cfg.MODEL.HEAD.NLP_VIEW_LOSS_MARGIN
                        if cfg.MODEL.HEAD.NLP_VIEW_SOFT:
                            nlp_view_loss = F.softplus(
                                logit_scale_nl * delta).mean()
                        else:
                            nlp_view_loss = F.relu(delta).mean()
                    else:
                        # contrastive
                        neg_sim = torch.diag(torch.matmul(
                            motion_lang_embeds, view_lang_embeds.T)).unsqueeze_(1)
                        if cfg.MODEL.HEAD.NLP_VIEW_SOFT:
                            nlp_view_loss = F.softplus(
                                logit_scale_nl * neg_sim).mean()
                        else:
                ...
                for cls_logit in cls_logits:
                    cls_loss += cfg.MODEL.HEAD.CLS_WEIGHT * \
                        cross_entropy(cls_logit, id_car.long().cuda(),
                                      epsilon=cfg.MODEL.HEAD.CE_EPSILON)

                # metric learning
                metric_loss = torch.tensor(0., device='cuda')
                if metric_learning != 'None':
                    for pair in loss_pairs:
                        metric_loss += metric_loss_f(torch.cat(pair),
                                                     torch.cat([id_car, id_car]).long().cuda())

                metric_loss *= cfg.MODEL.METRIC.METRIC_WEIGHT

                loss = clip_loss + cls_loss + metric_loss + nlp_view_loss

                acc1, acc5 = accuracy(acc_sim, torch.arange(
                    image.size(0)).cuda(), topk=(1, 5))
        ...
        print(f'Epoch {epoch} running time: ', timedelta(
            seconds=epo_end_time - epo_start_time))
        print(f'Logs dir: {args.logs_dir} ')

    del train_data, trainloader

    evaluate_by_test_all(model, valloader, cfg.TRAIN.EPOCH,
                         cfg, args.feat_idx, args, tokenizer, optimizer)
