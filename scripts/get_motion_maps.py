...
data_path = 'your-path-to-data/mine'
...
with open("your-path-to-data/test-tracks.json") as f:
    tracks_test = json.load(f)
with open("your-path-to-data/train-tracks.json") as f:
    tracks_train = json.load(f)
...
def get_motion_map(info):
    ...
    for i in range(len(track["frames"])):
        ...
        if i % 7 == 1:
            example[box[1]:box[1] + box[3], box[0]: box[0] + box[2],
                    :] = frame[box[1]:box[1] + box[3], box[0]: box[0] + box[2], :]

    words = track["frames"][0].split('/')
    bk_path = osp.join(data_path, "data/bk_map",
                       words[-4] + '_' + words[-3] + '.jpg')
    avg_img = cv2.imread(bk_path).astype(np.int)
    postions = (example[:, :, 0] == 0) & (
...
paths = ['train/S01', 'validation/S02',
