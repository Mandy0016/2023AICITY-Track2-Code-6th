...
with open("your-path-to-data/test-queries.json") as f:
    queries = json.load(f)
with open("your-path-to-data/test-tracks.json") as f:
    tracks = json.load(f)
                ...
                if len(car_lang_embeds) >= 2:
                    ...
                    for _, car_vis_embeds in track_car_embeds.items():
                        ...
                        for car_vis_embed in car_vis_embeds:
                            other_lang_embed = other_lang_embed / \
                                np.linalg.norm(other_lang_embed)
                            car_vis_embed = car_vis_embed / \
                                np.linalg.norm(car_vis_embed)
                            max_sim_val = max(max_sim_val, np.matmul(
                                other_lang_embed, car_vis_embed.T))
                        relation_sim.append(max_sim_val)
                    relation_sim = np.array(relation_sim)
                    relation_sim = relation_sim / \
                        np.max(relation_sim) * np.max(cur_sim) * 0.2
                    cur_sim = cur_sim + relation_sim

                loc_sim = np.matmul(
                    np.array([lang_loc_v[query]]), img_loc_logits.T)
                loc_sim = loc_sim.T
...
def main():
    ...
    feature_mean_score_test(args, config_file_list,
                            merge_weights, enforced=enforced, spatial=spatial)
